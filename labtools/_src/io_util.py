...
def maybe_rlocation(path: str) -> str:
  runfiles = maybe_import('rules_python.python.runfiles.runfiles')
  resolved_path = path
...
def resolve_path(path: Union[str, Path]) -> str:
  """ Resolves a path, potentially relative to some bazel-specific prefix

  Args:
    path: Path to be resolved. Certain prefixes have special meanings:
      `bazel::ws://`: Path relative to the Bazel workspace root. Note this does
        not work for tests.
      `bazel::wd://`: Path relative to the current working directory outside of
        a `bazel run`. Equivalent to passing `--arg=$PWD/filename` on the
        commandline. This does not work for tests.
      `bazel::rf://`: Path in the runfiles tree.

      Note that in the non-bazel setting paths such as `bazel::ws://`,
      `bazel::wd://` and `bazel::rf://` will all resolve relative to the
      current working directory. One should avoid using `bazel::rf://` over
      `bazel::ws://` wherever possible. The former will only allow declared
      data files, and thus is also suitable for tests and Remote Execution.

      All paths not containing a protocol will be normalized using `os.path.
      abspath`.
  """
  path = str(path)
  if path.startswith('bazel::ws://'):
    prefix = os.environ.get('BUILD_WORKSPACE_DIRECTORY', '')
    path = os.path.join(prefix, re.sub(r'^bazel::ws://', '', path))
  elif path.startswith('bazel::wd://'):
    prefix = os.environ.get('BUILD_WORKING_DIRECTORY', os.getcwd())
    path = os.path.join(prefix, re.sub(r'^bazel::wd://', '', path))
  elif path.startswith('bazel::rf://'):
    path = maybe_rlocation(re.sub(r'^bazel::rf://', '', path))
  # Check if the path has a protocol, if not then normalize with abspath
